...
void EffectSsDust_Spawn(GameState* game, u16 drawFlags, Vec3f* pos, Vec3f* velocity, Vec3f* accel,
    ...
    EffectSs_Spawn(game, EFFECT_SS_DUST, 128, &initParams);
}

void func_800B0DE0(GlobalContext* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel, Color_RGBA8* primColor,
                   Color_RGBA8* envColor, s16 scale, s16 scaleStep) {
    EffectSsDust_Spawn(game, 0, pos, velocity, accel, primColor, envColor, scale, scaleStep, 10, 0);
}

void func_800B0E48(GlobalContext* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel, Color_RGBA8* primColor,
                   Color_RGBA8* envColor, s16 scale, s16 scaleStep) {
    EffectSsDust_Spawn(game, 1, pos, velocity, accel, primColor, envColor, scale, scaleStep, 10, 0);
}

void func_800B0EB0(GlobalContext* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel, Color_RGBA8* primColor,
                   Color_RGBA8* envColor, s16 scale, s16 scaleStep, s16 life) {
    EffectSsDust_Spawn(game, 0, pos, velocity, accel, primColor, envColor, scale, scaleStep, life, 0);
}

void func_800B0F18(GlobalContext* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel, Color_RGBA8* primColor,
                   Color_RGBA8* envColor, s16 scale, s16 scaleStep, s16 life) {
    EffectSsDust_Spawn(game, 1, pos, velocity, accel, primColor, envColor, scale, scaleStep, life, 0);
}

void func_800B0F80(GlobalContext* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel, Color_RGBA8* primColor,
                   Color_RGBA8* envColor, s16 scale, s16 scaleStep, s16 life) {
    EffectSsDust_Spawn(game, 2, pos, velocity, accel, primColor, envColor, scale, scaleStep, life, 0);
}

void func_800B0FE8(GlobalContext* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel, Color_RGBA8* primColor,
                   Color_RGBA8* envColor, s16 scale, s16 scaleStep) {
    EffectSsDust_Spawn(game, 0, pos, velocity, accel, primColor, envColor, scale, scaleStep, 10, 1);
}

void func_800B1054(GlobalContext* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel, Color_RGBA8* primColor,
                   Color_RGBA8* envColor, s16 scale, s16 scaleStep) {
    EffectSsDust_Spawn(game, 1, pos, velocity, accel, primColor, envColor, scale, scaleStep, 10, 1);
}

extern Color_RGBA8 D_801AE3B0;
extern Color_RGBA8 D_801AE3B4;

void func_800B10C0(GlobalContext* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel) {
    EffectSsDust_Spawn(game, 4, pos, velocity, accel, &D_801AE3B0, &D_801AE3B4, 100, 5, 10, 0);
}

void func_800B1130(GlobalContext* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel) {
    EffectSsDust_Spawn(game, 5, pos, velocity, accel, &D_801AE3B0, &D_801AE3B4, 100, 5, 10, 0);
}

void func_800B11A0(GlobalContext* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 scale, s16 scaleStep) {
    EffectSsDust_Spawn(game, 4, pos, velocity, accel, &D_801AE3B0, &D_801AE3B4, scale, scaleStep, 10, 0);
}

void func_800B1210(GlobalContext* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 scale, s16 scaleStep) {
    EffectSsDust_Spawn(game, 5, pos, velocity, accel, &D_801AE3B0, &D_801AE3B4, scale, scaleStep, 10, 0);
}

void func_800B1280(GlobalContext* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 scale, s16 scaleStep,
                   s16 life) {
    EffectSsDust_Spawn(game, 4, pos, velocity, accel, &D_801AE3B0, &D_801AE3B4, scale, scaleStep, life, 0);
}
void func_800B12F0(GlobalContext* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 scale, s16 scaleStep,
                   s16 life) {
    EffectSsDust_Spawn(game, 5, pos, velocity, accel, &D_801AE3B0, &D_801AE3B4, scale, scaleStep, life, 0);
}

void func_800B1360(GlobalContext* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel, Color_RGBA8* primColor,
                   Color_RGBA8* envColor) {
    func_800B0DE0(game, pos, velocity, accel, primColor, envColor, 100, 5);
}

void func_800B139C(GlobalContext* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel, Color_RGBA8* primColor,
                   Color_RGBA8* envColor) {
    func_800B0E48(game, pos, velocity, accel, primColor, envColor, 100, 5);
}
...
void func_800B14D4(GlobalContext* game, f32 randScale, Vec3f* srcPos) {
    s32 i;
    Vec3f pos;
    Vec3f velocity;
    Vec3f accel;
    ...
    for (i = 0; i < 20; i++) {
        ...
        func_800B1280(game, &pos, &velocity, &accel, 100, 30, 7);
    }
}

void func_800B1598(GlobalContext* game, f32 randScale, Vec3f* srcPos) {
    s32 i;
    Vec3f pos;
    Vec3f velocity;
    Vec3f accel;
    ...
    for (i = 0; i < 20; i++) {
        func_800B13D8(srcPos, randScale, &pos, &velocity, &accel);
        func_800B12F0(game, &pos, &velocity, &accel, 100, 30, 7);
    }
}
...
void EffectSsKiraKira_SpawnSmallYellow(GameState* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel) {
    ...
    EffectSsKiraKira_SpawnDispersed(game, pos, velocity, accel, &primColor, &envColor, 1000, 16);
}
...
void EffectSsKiraKira_SpawnDispersed(GameState* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel,
    ...
    EffectSs_Spawn(game, EFFECT_SS_KIRAKIRA, 128, &initParams);
}

void EffectSsKiraKira_SpawnFocused(GameState* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel,
                                   Color_RGBA8* primColor, Color_RGBA8* envColor, s16 scale, s32 life) {
    EffectSsKiraKiraInitParams initParams;
...
void EffectSsBomb2_SpawnFade(GameState* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel) {
    ...
    EffectSs_Spawn(game, EFFECT_SS_BOMB2, 10, &initParams);
}

void EffectSsBomb2_SpawnLayered(GameState* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 scale,
                                s16 scaleStep) {
    EffectSsBomb2InitParams initParams;
...
void EffectSsBlast_Spawn(GameState* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel, Color_RGBA8* primColor,
    ...
    EffectSs_Spawn(game, EFFECT_SS_BLAST, 128, &initParams);
}

extern Color_RGBA8 D_801AE3C0;
extern Color_RGBA8 D_801AE3C4;

void EffectSsBlast_SpawnWhiteCustomScale(GameState* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 scale,
                                         s16 scaleStep, s16 life) {
    EffectSsBlast_Spawn(game, pos, velocity, accel, &D_801AE3C0, &D_801AE3C4, scale, scaleStep, 35, life);
}

void EffectSsBlast_SpawnShockwave(GameState* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel,
                                  Color_RGBA8* primColor, Color_RGBA8* envColor, s16 life) {
    EffectSsBlast_Spawn(game, pos, velocity, accel, primColor, envColor, 100, 375, 35, life);
}

extern Color_RGBA8 D_801AE3C8;
extern Color_RGBA8 D_801AE3CC;

void EffectSsBlast_SpawnWhiteShockwave(GameState* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel) {
    EffectSsBlast_SpawnShockwave(game, pos, velocity, accel, &D_801AE3C8, &D_801AE3CC, 10);
}

// EffectSsGSpk Spawn Functions

void EffectSsGSpk_SpawnAccel(GameState* game, Actor* actor, Vec3f* pos, Vec3f* velocity, Vec3f* accel,
    ...
    EffectSs_Spawn(game, EFFECT_SS_G_SPK, 128, &initParams);
}

// unused
void EffectSsGSpk_SpawnNoAccel(GameState* game, Actor* actor, Vec3f* pos, Vec3f* velocity, Vec3f* accel,
                               Color_RGBA8* primColor, Color_RGBA8* envColor, s16 scale, s16 scaleStep) {
    EffectSsGSpkInitParams initParams;
...
void EffectSsGSpk_SpawnFuse(GameState* game, Actor* actor, Vec3f* pos, Vec3f* velocity, Vec3f* accel) {
    Color_RGBA8 primColor = D_801AE3D0;
    Color_RGBA8 envColor = D_801AE3D4;

    EffectSsGSpk_SpawnSmall(game, actor, pos, velocity, accel, &primColor, &envColor);
}
...
// unused
void EffectSsGSpk_SpawnRandColor(GameState* game, Actor* actor, Vec3f* pos, Vec3f* velocity, Vec3f* accel,
    ...
    EffectSsGSpk_SpawnAccel(game, actor, pos, velocity, accel, &primColor, &envColor, scale, scaleStep);
}

void EffectSsGSpk_SpawnSmall(GameState* game, Actor* actor, Vec3f* pos, Vec3f* velocity, Vec3f* accel,
                             Color_RGBA8* primColor, Color_RGBA8* envColor) {
    EffectSsGSpk_SpawnAccel(game, actor, pos, velocity, accel, primColor, envColor, 100, 5);
}

// EffectSsDFire Spawn Functions
#ifdef NON_MATCHING
void EffectSsDFire_Spawn(GameState* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 scale, s16 scaleStep,
    ...
    EffectSs_Spawn(game, EFFECT_SS_D_FIRE, 128, &initParams);
}
#else
...
void EffectSsBubble_Spawn(GameState* game, Vec3f* pos, f32 yPosOffset, f32 yPosRandScale, f32 xzPosRandScale,
                          f32 scale) {
    ...
    EffectSs_Spawn(game, EFFECT_SS_BUBBLE, 128, &initParams);
}

// EffectSsGRipple Spawn Functions

void EffectSsGRipple_Spawn(GameState* game, Vec3f* pos, s16 radius, s16 radiusMax, s16 life) {
    ...
    EffectSs_Spawn(game, EFFECT_SS_G_RIPPLE, 128, &initParams);
}

// EffectSsGSplash Spawn Functions

void EffectSsGSplash_Spawn(GameState* game, Vec3f* pos, Color_RGBA8* primColor, Color_RGBA8* envColor,
                           s16 type, s16 scale) {
    ...
    EffectSs_Spawn(game, EFFECT_SS_G_SPLASH, 128, &initParams);
}

// EffectSsGFire Spawn Functions

void EffectSsGFire_Spawn(GameState* game, Vec3f* pos) {
    EffectSsGFireInitParams initParams;

    Math_Vec3f_Copy(&initParams.pos, pos);

    EffectSs_Spawn(game, EFFECT_SS_G_FIRE, 128, &initParams);
}

// EffectSsLightning Spawn Functions

void EffectSsLightning_Spawn(GameState* game, Vec3f* pos, Color_RGBA8* primColor, Color_RGBA8* envColor,
    ...
    EffectSs_Spawn(game, EFFECT_SS_LIGHTNING, 128, &initParams);
}

// EffectSsDtBubble Spawn Functions

void EffectSsDtBubble_SpawnColorProfile(GameState* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 scale,
    ...
    EffectSs_Spawn(game, EFFECT_SS_DT_BUBBLE, 128, &initParams);
}

void EffectSsDtBubble_SpawnCustomColor(GameState* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel,
 ...
 */
void EffectSsHahen_Spawn(GameState* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 unused, s16 scale,
    ...
    EffectSs_Spawn(game, EFFECT_SS_HAHEN, 128, &initParams);
}
 ...
 */
#ifdef NON_MATCHING
void EffectSsHahen_SpawnBurst(GameState* game, Vec3f* pos, f32 burstScale, s16 unused, s16 scale,
    ...
    for (i = 0; i < count; i++) {
        ...
        EffectSsHahen_Spawn(game, pos, &velocity, &accel, unused, Rand_S16Offset(scale, randScaleRange), objId,
                            life, dList);
    }
}
#else
...
void func_800B2364(GlobalContext* game, Vec3f* pos, Gfx* dList) {
    Vec3f posVec = D_801AE3E0;

    EffectSsHahen_Spawn(game, pos, &D_801D15B0, &posVec, 1, 5, 1, 10, dList);
}
 ...
 */
void EffectSsStick_Spawn(GameState* game, Vec3f* pos, s16 yaw) {
    EffectSsStickInitParams initParams;

    initParams.pos = *pos;
    initParams.yaw = yaw;

    EffectSs_Spawn(game, EFFECT_SS_STICK, 128, &initParams);
}

// EffectSsSibuki Spawn Functions

void EffectSsSibuki_Spawn(GameState* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 moveDelay,
    ...
    EffectSs_Spawn(game, EFFECT_SS_SIBUKI, 128, &initParams);
}

extern Vec3f D_801AE3EC;
extern Vec3f D_801AE3F8;

void EffectSsSibuki_SpawnBurst(GameState* game, Vec3f* pos) {
    s16 i;
    Vec3f zeroVec = D_801AE3EC;
    s16 randDirection = Rand_ZeroOne() * 1.99f;

    for (i = 0; i < KREG(19) + 30; i++) {
        EffectSsSibuki_Spawn(game, pos, &zeroVec, &zeroVec, i / (KREG(27) + 6), randDirection, KREG(18) + 40);
    }
}

// EffectSsStone1 Spawn Functions

void EffectSsStone1_Spawn(GameState* game, Vec3f* pos, s32 arg2) {
    EffectSsStone1InitParams initParams;

    initParams.pos = *pos;
    initParams.unk_C = arg2;

    EffectSs_Spawn(game, EFFECT_SS_STONE1, 128, &initParams);
}

// EffectSsHitMark Spawn Functions

void EffectSsHitMark_Spawn(GameState* game, s32 type, s16 scale, Vec3f* pos) {
    ...
    EffectSs_Spawn(game, EFFECT_SS_HITMARK, 128, &initParams);
}

void EffectSsHitMark_SpawnFixedScale(GameState* game, s32 type, Vec3f* pos) {
    EffectSsHitMark_Spawn(game, type, 300, pos);
}

void EffectSsHitMark_SpawnCustomScale(GameState* game, s32 type, s16 scale, Vec3f* pos) {
    EffectSsHitMark_Spawn(game, type, scale, pos);
}
 ...
 */
void EffectSsFhgFlash_SpawnShock(GameState* game, Actor* actor, Vec3f* pos, s16 scale, u8 param) {
    ...
    EffectSs_Spawn(game, EFFECT_SS_FHG_FLASH, 128, &initParams);
}

// EffectSsKFire Spawn Functions

void EffectSsKFire_Spawn(GameState* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 scaleMax, u8 type) {
    ...
    EffectSs_Spawn(game, EFFECT_SS_K_FIRE, 128, &initParams);
}

// EffectSsSolderSrchBall Spawn Functions

void EffectSsSolderSrchBall_Spawn(GameState* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 unused,
    ...
    EffectSs_Spawn(game, EFFECT_SS_SOLDER_SRCH_BALL, 128, &initParams);
}

// EffectSsKakera Spawn Functions

void EffectSsKakera_Spawn(GameState* game, Vec3f* pos, Vec3f* velocity, Vec3f* arg3, s16 gravity, s16 arg5,
    ...
    EffectSs_Spawn(game, EFFECT_SS_KAKERA, 101, &initParams);
}

// EffectSsIcePiece Spawn Functions

void EffectSsIcePiece_Spawn(GameState* game, Vec3f* pos, f32 scale, Vec3f* velocity, Vec3f* accel, s32 life) {
    ...
    initParams.scale = scale;
    initParams.life = life;
    EffectSs_Spawn(game, EFFECT_SS_ICE_PIECE, 128, &initParams);
}
...
void EffectSsIcePiece_SpawnBurst(GameState* game, Vec3f* refPos, f32 scale) {
    ...
    for (i = 0; i < ARRAY_COUNT(vecScales); i++) {
        ...
        EffectSsIcePiece_Spawn(game, &pos, (Rand_ZeroFloat(1.0f) + 0.5f) * ((scale * 1.3f) * 100.0f), &velocity,
                               &accel, 25);
    }
}
#else
...
void EffectSsEnIce_SpawnFlyingVec3f(GameState* game, Actor* actor, Vec3f* pos, Color_RGBA8* prim,
    ...
    EffectSs_Spawn(game, EFFECT_SS_EN_ICE, 80, &initParams);
}

extern Color_RGBA8 D_801AE47C;
extern Color_RGBA8 D_801AE480;

void func_800B2B44(GlobalContext* game, Actor* actor, Vec3f* pos, f32 scale) {
    EffectSsEnIce_SpawnFlyingVec3f(game, actor, pos, &D_801AE47C, &D_801AE480, scale);
}

void func_800B2B7C(GlobalContext* game, Actor* actor, Vec3s* arg2, f32 scale) {
    Vec3f dest;

    Math_Vec3s_ToVec3f(&dest, arg2);
    func_800B2B44(game, actor, &dest, scale);
}

void EffectSsEnIce_Spawn(GameState* game, Vec3f* pos, f32 scale, Vec3f* velocity, Vec3f* accel,
    ...
    EffectSs_Spawn(game, EFFECT_SS_EN_ICE, 128, &initParams);
}

// EffectSsFireTail Spawn Functions

void EffectSsFireTail_Spawn(GameState* game, Actor* actor, Vec3f* pos, f32 scale, Vec3f* arg4, s16 arg5,
    ...
    EffectSs_Spawn(game, EFFECT_SS_FIRE_TAIL, 128, &initParams);
}
...
#ifdef NON_MATCHING
// needs data migration to match
void EffectSsFireTail_SpawnFlame(GameState* game, Actor* actor, Vec3f* pos, f32 arg3, s16 bodyPart,
                                 f32 colorIntensity) {
    ...
    EffectSsFireTail_Spawn(game, actor, pos, arg3, &actor->velocity, 15, &D_801AE484, &D_801AE488,
...
void EffectSsFireTail_SpawnFlameOnPlayer(GameState* game, f32 scale, s16 bodyPart, f32 colorIntensity) {
    GlobalContext* globalCtx = (GlobalContext*)game;
    Player* player = PLAYER;

    EffectSsFireTail_SpawnFlame(game, &player->actor, &player->bodyPartsPos[bodyPart], scale, bodyPart,
                                colorIntensity);
}

// EffectSsEnFire Spawn Functions

void EffectSsEnFire_SpawnVec3f(GameState* game, Actor* actor, Vec3f* pos, s16 scale, s16 arg4, s16 flags,
                               s16 bodyPart) {
    ...
    EffectSs_Spawn(game, EFFECT_SS_EN_FIRE, 128, &initParams);
}

void EffectSsEnFire_SpawnVec3s(GameState* game, Actor* actor, Vec3s* pos, s16 scale, s16 arg4, s16 flags,
                               s16 bodyPart) {
    EffectSsEnFireInitParams initParams;
...
void EffectSsExtra_Spawn(GameState* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 scale, s16 scoreIdx) {
    ...
    EffectSs_Spawn(game, EFFECT_SS_EXTRA, 100, &initParams);
}

// EffectSsDeadDb Spawn Functions
void EffectSsDeadDb_Spawn(GameState* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel, Color_RGBA8* prim,
    ...
    EffectSs_Spawn(game, EFFECT_SS_DEAD_DB, 120, &initParams);
}

extern Color_RGBA8 D_801AE48C;
extern Color_RGBA8 D_801AE490[4];

void func_800B3030(GlobalContext* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 scale, s16 scaleStep,
                   s32 colorIndex) {
    EffectSsDeadDb_Spawn(game, pos, velocity, accel, &D_801AE48C, &D_801AE490[colorIndex], scale, scaleStep, 9);
}

// EffectSsDeadDd Spawn Functions

void EffectSsDeadDd_Spawn(GameState* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel, Color_RGBA8* prim,
    ...
    EffectSs_Spawn(game, EFFECT_SS_DEAD_DD, 120, &initParams);
}

// EffectSsDeadDs Spawn Functions

void EffectSsDeadDs_Spawn(GameState* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 scale, s16 scaleStep,
                          s16 alpha, s32 life) {
    ...
    initParams.alpha = alpha;
    initParams.life = life;
    EffectSs_Spawn(game, EFFECT_SS_DEAD_DS, 100, &initParams);
}

void func_800B31BC(GlobalContext* game, Vec3f* pos, s16 scale, s16 scaleStep, s16 alpha, s32 life) {
    EffectSsDeadDs_Spawn(game, pos, &D_801D15B0, &D_801D15B0, scale, scaleStep, alpha, life);
}

// EffectSsIceSmoke Spawn Functions

void EffectSsIceSmoke_Spawn(GameState* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 scale) {
    ...
    EffectSs_Spawn(game, EFFECT_SS_ICE_SMOKE, 128, &initParams);
}

// EffectSsIceBlock Spawn Functions

void EffectSsIceBlock_Spawn(GameState* game, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 scale) {
    ...
    EffectSs_Spawn(game, EFFECT_EN_ICE_BLOCK, 128, &initParams);
}
