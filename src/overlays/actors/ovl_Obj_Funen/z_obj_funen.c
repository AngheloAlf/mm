...
void ObjFunen_Init(Actor* thisx, GameState* game);
void ObjFunen_Draw(Actor* thisx, GameState* game);
...
void ObjFunen_Draw(Actor* thisx, GameState* game) {
    ...
    gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(game->gfxCtx), G_MTX_MODELVIEW | G_MTX_LOAD | G_MTX_NOPUSH);

    temp = -(((GlobalContext*)game)->gameplayFrames & 0x7FFFFFFF) & 0x7F;

    gSPSegment(POLY_XLU_DISP++, 0x08,
               Gfx_TwoTexScroll(game->gfxCtx, 0, 0, temp, 0x20, 0x20, 1, 0, temp, 0x20, 0x20));
