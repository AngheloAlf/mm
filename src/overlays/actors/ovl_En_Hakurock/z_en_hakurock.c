...
void func_80B21BE0(BossHakugin* parent, Vec3f* arg1, s32 arg2) {
    ...
    for (i = 0; i < ARRAY_COUNT(parent->unk_9F8); i++) {
        ...
        if (gohtEffect->unk_18 < 0) {
            ...
            gohtEffect->unk_1C.x = (s32)qrand() >> 0x10;
            gohtEffect->unk_1C.y = (s32)qrand() >> 0x10;
            gohtEffect->unk_1C.z = (s32)qrand() >> 0x10;
...
void func_80B220A8(EnHakurock* this) {
    ...
    this->actor.world.rot.y = ((s32)qrand() >> 0x12) + this->actor.parent->shape.rot.y + 0x8000;
    this->actor.shape.rot.x = (s32)qrand() >> 0x10;
    this->actor.shape.rot.y = (s32)qrand() >> 0x10;
    this->actor.shape.rot.z = (s32)qrand() >> 0x10;
...
void func_80B222AC(EnHakurock* this, PlayState* play) {
    ...
    angle = ((s32)qrand() >> 0x13) + player->actor.shape.rot.y;
    this->actor.shape.rot.y = (s32)qrand() >> 0x10;
